...
class CaaparserValidationException implements Exception {
  final List<String> errors;

  CaaparserValidationException(this.errors);

  @override
  String toString() => 'CaaparserValidationException: ${errors.join("; ")}';
}
...
/// CAA Record Parser API client.
///
/// For more information, visit: https://apiverve.com/marketplace/caaparser?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [record] (required) - The CAA record string to parse
class CaaparserClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'record': ValidationRule(type: 'string', required: true),
  };

  CaaparserClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/caaparser',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CaaparserValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CaaparserValidationException(errors);
    }
  }

  /// Execute a request to the CAA Record Parser API.
  ///
  /// Parameters are validated before sending the request.
  Future<CaaparserResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CaaparserException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CaaparserResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CaaparserException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CaaparserException('Resource not found');
      } else {
        throw CaaparserException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CaaparserException) rethrow;
      throw CaaparserException('Request failed: $e');
    }
  }
...
/// Exception thrown by the CAA Record Parser API client.
class CaaparserException implements Exception {
  final String message;

  CaaparserException(this.message);

  @override
  String toString() => 'CaaparserException: $message';
}
